      ...
      async function (req, Email, password, done) {
        ...
        if (req.params.admin == "adminmanage") {
            ...
            await Admin.findOne({ Email: Email }, function (err, user) {
              ...
              //checks for password
                ...
                return done(
                  null,
                  false,
                  req.flash("primary", "password do not match")
                );
              }
            }).clone();
          } catch (err) {
          ...
          await User.findOne({ Email: Email }, async (err, user) => {
            // console.log(user, Email, "called two");
            if (err) return done(err);
            //check if user exist
            if (!user) {
              ...
              return done(
                null,
                false,
                req.flash("primary", "No such Email Registerd")
              );
            }
            ...
            if (await bcrypt.compare(password, user.password)) {
              ...
              return done(
                null,
                false,
                req.flash("primary", "passwords do not match")
              );
            }
          }).clone();
          //end of client check
