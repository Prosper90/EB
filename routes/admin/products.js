...
//Edit products
router.post("/edit/:id", checkAuthenticated, async function (req, res) {
  ...
  await Products.findById({ _id: req.params.id }, function (err, product) {
    ...
    product.save(function (saveerr, saveresult) {
      if (saveerr) {
        ...
        // }
        req.flash("primary", "Something went wrong");
...
//remove products
router.get("/remove/:id", checkAuthenticated, async (req, res) => {
  try {
    ...
    if (err) {
      ...
      user.Deposit.map((datatoset) => {
        ...
        user.Notification.push({
          message: `Deposit of $ ${datatoset.amount} successful`,
        });
      });
